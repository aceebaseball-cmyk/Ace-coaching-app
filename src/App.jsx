...
const emptyProfile = {
 ...
 const saveAthlete = async () => {
    ...
    const newAthlete = {
      ...
      plans: [],
      activeWeekId: null,
      veloLog: newAthleteProfile.currentVelo ? [{ date: new Date().toLocaleDateString(), velo: newAthleteProfile.currentVelo }] : [],
    };
...
function Assessment({ activeAthlete, profile, setProfile, primaryIssues, setPrimaryIssues, secondaryIssues, setSecondaryIssues, athleteType, setAthleteType, primarySoreness, setPrimarySoreness, toggle, saveAssessmentToAthlete }) {
  return (
    <section className="grid lg:grid-cols-3 gap-5">
      <div className="lg:col-span-1 rounded-3xl bg-zinc-950 border border-zinc-800 p-5">
        <h2 className="text-xl font-black mb-2">Assessment Athlete</h2>
        ...
        <button onClick={saveAssessmentToAthlete} disabled={!activeAthlete || primaryIssues.length === 0} className="w-full bg-white text-black rounded-xl py-3 font-black disabled:opacity-40">Save Assessment</button>
      </div>
      <div className="lg:col-span-2 rounded-3xl bg-zinc-950 border border-zinc-800 p-5">
        ...
        {assessmentSections.filter((s) => s.tags).map((s) => <div key={s.title} className="mb-5"><h3 className="font-black text-red-400 mb-2">{s.title}</h3><div className="flex flex-wrap gap-2">{s.tags.map((t) => { const isType = s.title === "Athlete Type"; const isPrimary = primaryIssues.includes(t); const isSecondary = secondaryIssues.includes(t); const isAthleteType = athleteType.includes(t); return <button key={t} onClick={() => { if (isType) return toggle(t, athleteType, setAthleteType); if (isPrimary) { setPrimaryIssues(primaryIssues.filter((x) => x !== t)); setSecondaryIssues([...secondaryIssues, t]); } else if (isSecondary) { setSecondaryIssues(secondaryIssues.filter((x) => x !== t)); } else { setPrimaryIssues([...primaryIssues, t]); } }} className={`rounded-full border px-3 py-2 text-sm ${isPrimary ? "bg-red-700 border-red-500" : isSecondary ? "bg-zinc-700 border-zinc-500" : isAthleteType ? "bg-white text-black border-white" : "bg-black border-zinc-800"}`}>{t}</button>; })}</div></div>)}
      </div>
    </section>
  );
}
